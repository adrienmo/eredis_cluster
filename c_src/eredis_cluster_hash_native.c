...
hash(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if (enif_get_string(env, argv[0], ns, length+1, ERL_NIF_LATIN1) < 1) {
        return mk_error(env, "not_a_list");
    }

    crc = keyHashSlot(ns,length);
...
ERL_NIF_INIT(eredis_cluster_hash_native, nif_funcs, NULL, NULL, NULL, NULL);
